<template>
    ...
    <section class="p-3">
        <div class="card p-5 mb-2 min-h-[16rem] flex flex-col items-center justify-between">
            ...
            <NuxtLink v-if="resNotify?.topic != 'show' && resNotify?.topic == 'jobs_process'"
                :to="`/jobs/mission/${resNotify?.value_id}`">
                <TmmButton label="ดูรายละเอียด" type="primary" />
            </NuxtLink>
            <NuxtLink v-if="resNotify?.topic != 'show' && resNotify?.topic == 'report'"
                :to="`/news/post/${resNotify?.value_id}`">
                <TmmButton label="ดูรายละเอียด" type="primary" />
            </NuxtLink>
